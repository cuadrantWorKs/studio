...
export async function syncLocalDataToSupabase() {
  ...
  try {
    ...
    // Sync Workdays
    const unsyncedWorkdays = await localDb.workdays
      .filter((item: Workday) => !item.isSynced)
      .toArray();
    if (unsyncedWorkdays.length > 0) {
      console.log(
        `Attempting to sync ${unsyncedWorkdays.length} unsynced workdays.`,
      );
      const workdaysToUpsert = unsyncedWorkdays.map((workday: Workday) => ({
        ...
        date: workday.date,
        start_time:
          typeof workday.startTime === "number"
            ? new Date(workday.startTime).toISOString()
            : null,
        start_location: workday.startLocation ?? null,
        end_time:
          typeof workday.endTime === "number"
            ? new Date(workday.endTime).toISOString()
            : null,
      ...
      const { error: workdayError } = await supabaseDb
        .from("workdays")
        .upsert(workdaysToUpsert, { onConflict: "id" });
      if (workdayError) {
    ...
    // Sync Locations
    const unsyncedLocations = await localDb.locations
      .filter(
        (
          item: LocationPoint & {
            workdayId: string;
    ...
    if (unsyncedLocations.length > 0) {
      console.log(
        `Attempting to sync ${unsyncedLocations.length} unsynced locations.`,
      );
      const locationsToInsert = unsyncedLocations.map(
        (
      ...
      const { error: locationsError } = await supabaseDb
        .from("locations")
    ...
    // Sync Jobs
    const unsyncedJobs = await localDb.jobs
      .filter((item: Job) => !item.isSynced)
      .toArray();
    if (unsyncedJobs.length > 0) {
      ...
      const { error: jobsError } = await supabaseDb
        .from("jobs")
    ...
    const unsyncedPauseIntervals = await localDb.pauseIntervals
      .filter((item: PauseInterval) => !item.isSynced)
      .toArray();
    if (unsyncedPauseIntervals.length > 0) {
      console.log(
        `Attempting to sync ${unsyncedPauseIntervals.length} unsynced pause intervals.`,
      );
      const pauseIntervalsToUpsert = unsyncedPauseIntervals.map(
        (pauseInterval: PauseInterval) => ({
          id: pauseInterval.id,
          workday_id: pauseInterval.workdayId,
          start_time:
            typeof pauseInterval.startTime === "number"
              ? new Date(pauseInterval.startTime).toISOString()
              : null,
          start_location: pauseInterval.startLocation ?? null,
          end_time:
            typeof pauseInterval.endTime === "number"
              ? new Date(pauseInterval.endTime).toISOString()
              : null,
          end_location: pauseInterval.endLocation ?? null, // Assuming DbPauseInterval already has LocationPoint | null
          is_synced: true, // Mark as synced in the data being sent
        }),
      );
      const { error: pausesError } = await supabaseDb
        .from("pause_intervals")
        .upsert(pauseIntervalsToUpsert, { onConflict: "id" });
      if (pausesError) {
        console.error(
          "Error syncing pause intervals to Supabase:",
          pausesError,
        );
      } else {
        console.log(
          `${unsyncedPauseIntervals.length} pause intervals synced successfully.`,
        );
        // Mark successfully synced pause intervals in local DB
        for (const pauseInterval of unsyncedPauseIntervals) {
          await localDb.pauseIntervals.update(pauseInterval.id!, {
            isSynced: true,
          });
        }
      }
    ...
    // Sync Events
    const unsyncedEvents = await localDb.events
      .filter((item: TrackingEvent) => !item.isSynced)
      .toArray();
    if (unsyncedEvents.length > 0) {
      console.log(
        `Attempting to sync ${unsyncedEvents.length} unsynced events.`,
      );
